...
import static io.awesome.gagtube.player.MainPlayer.ACTION_CLOSE;
import static io.awesome.gagtube.player.MainPlayer.ACTION_FAST_FORWARD;
import static io.awesome.gagtube.player.MainPlayer.ACTION_FAST_REWIND;
import static io.awesome.gagtube.player.MainPlayer.ACTION_PLAY_NEXT;
import static io.awesome.gagtube.player.MainPlayer.ACTION_PLAY_PAUSE;
import static io.awesome.gagtube.player.MainPlayer.ACTION_PLAY_PREVIOUS;
import static io.awesome.gagtube.player.MainPlayer.ACTION_REPEAT;
import static io.awesome.gagtube.player.MainPlayer.ACTION_SHUFFLE;
...
public final class NotificationUtil {
    ...
    private synchronized NotificationCompat.Builder createNotification(final VideoPlayerImpl player) {
        ...
        final NotificationCompat.Builder builder = new NotificationCompat.Builder(player.context, player.context.getString(R.string.notification_channel_id));
        ...
        builder.setStyle(new androidx.media.app.NotificationCompat.MediaStyle()
                ...
                .setDeleteIntent(PendingIntent.getBroadcast(player.context, NOTIFICATION_ID, new Intent(ACTION_CLOSE), FLAG_IMMUTABLE));
    ...
    private NotificationCompat.Action getAction(final VideoPlayerImpl player, @NotificationConstants.Action final int selectedAction) {
        ...
        switch (selectedAction) {
            case NotificationConstants.PREVIOUS:
                return getAction(player, baseActionIcon, R.string.exo_controls_previous_description, ACTION_PLAY_PREVIOUS);

            case NotificationConstants.NEXT:
                return getAction(player, baseActionIcon, R.string.exo_controls_next_description, ACTION_PLAY_NEXT);

            case NotificationConstants.REWIND:
                return getAction(player, baseActionIcon, R.string.exo_controls_rewind_description, ACTION_FAST_REWIND);

            case NotificationConstants.FORWARD:
                return getAction(player, baseActionIcon, R.string.exo_controls_fastforward_description, ACTION_FAST_FORWARD);

            case NotificationConstants.SMART_REWIND_PREVIOUS:
                if (player.playQueue != null && player.playQueue.size() > 1) {
                    return getAction(player, R.drawable.ic_control_previous_white_24dp, R.string.exo_controls_previous_description, ACTION_PLAY_PREVIOUS);
                } else {
                    return getAction(player, R.drawable.ic_fast_rewind, R.string.exo_controls_rewind_description, ACTION_FAST_REWIND);
                }

            case NotificationConstants.SMART_FORWARD_NEXT:
                if (player.playQueue != null && player.playQueue.size() > 1) {
                    return getAction(player, R.drawable.ic_control_next_white_24dp, R.string.exo_controls_next_description, ACTION_PLAY_NEXT);
                } else {
                    return getAction(player, R.drawable.ic_fast_forward, R.string.exo_controls_fastforward_description, ACTION_FAST_FORWARD);
                }
            ...
            case NotificationConstants.PLAY_PAUSE:
                if (player.getCurrentState() == BasePlayer.STATE_COMPLETED) {
                    return getAction(player, R.drawable.ic_replay_white_24dp, R.string.exo_controls_pause_description, ACTION_PLAY_PAUSE);
                    ...
                    return getAction(player, R.drawable.ic_pause_white_24dp, R.string.exo_controls_pause_description, ACTION_PLAY_PAUSE);
                } else {
                    return getAction(player, R.drawable.ic_play_arrow_white_24dp, R.string.exo_controls_play_description, ACTION_PLAY_PAUSE);
                }

            case NotificationConstants.REPEAT:
                if (player.getRepeatMode() == REPEAT_MODE_ALL) {
                    return getAction(player, R.drawable.controls_repeat_all, R.string.exo_controls_repeat_all_description, ACTION_REPEAT);
                } else if (player.getRepeatMode() == REPEAT_MODE_ONE) {
                    return getAction(player, R.drawable.controls_repeat_one, R.string.exo_controls_repeat_one_description, ACTION_REPEAT);
                } else /* player.getRepeatMode() == REPEAT_MODE_OFF */ {
                    return getAction(player, R.drawable.controls_repeat_off, R.string.exo_controls_repeat_off_description, ACTION_REPEAT);
                }

            case NotificationConstants.SHUFFLE:
                if (player.playQueue != null && player.playQueue.isShuffled()) {
                    return getAction(player, R.drawable.ic_shuffle_white_24dp, R.string.exo_controls_shuffle_on_description, ACTION_SHUFFLE);
                } else {
                    return getAction(player, R.drawable.ic_shuffle_dark_24dp, R.string.exo_controls_shuffle_off_description, ACTION_SHUFFLE);
                }

            case NotificationConstants.CLOSE:
                return getAction(player, R.drawable.ic_close_white_24dp, R.string.close, ACTION_CLOSE);
